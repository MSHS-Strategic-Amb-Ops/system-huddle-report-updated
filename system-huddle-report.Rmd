...
#   group_by(SITE, Appt.Year, Appt.MonNum, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PATIENT_CLASS, CLASS, radiology_check) %>%
#   summarise(total = n()) %>%
#   collect()
# 
# 
...
#   filter(DERIVED_STATUS_DESC == "Arrived") %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME,  DEPARTMENT_NAME, appt_year, appt_month, radiology_check, CLASS, PATIENT_CLASS) %>%
#   summarise(total = n()) %>%
#   collect()
# 
...
#   left_join(radiology_dep_mapping_tbl %>% select(DEPARTMENT_ID, CLASS),
#             by = c("DEPARTMENT_ID")) %>%
#   mutate(radiology_check = ifelse((CLASS == "OP") | (CLASS == "IP_OP" & PATIENT_CLASS %LIKE% '%OUTPATIENT%') | (CLASS == "IP_OP" & is.na(PATIENT_CLASS)), "Include", "Exclude")) %>%
#   mutate(radiology_check = ifelse(is.na(radiology_check), "Include", radiology_check)) %>%
#   filter(radiology_check == "Include") %>%
#   filter(!is.na(SITE)) %>%
#   mutate(Campus = SITE) %>%
