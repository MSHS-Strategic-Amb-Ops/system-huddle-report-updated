...
check <- access_raw_tbl %>%
  filter(PAT_ENC_CSN_ID == "100174756129") %>%
  collect()
...
#   filter(DERIVED_STATUS_DESC == "Arrived") %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, Appt.Year, Appt.MonNum, CONTACT_DATE, VISIT_GROUP_NUM) %>%
#   summarise(total = n()) %>%
#   collect() %>%
#   mutate(Appt.Month = format(as.Date(paste0(Appt.MonNum, "-01-", Appt.Year), "%m-%d-%Y"), "%b")) %>%
#   mutate(Appt.Year = as.character(Appt.Year))
...
# 
# data_list <- list("EPIC" = data,
#                   "Rad and RadOnc" = rad_radOnc_data)
# 
# require(openxlsx)
# write.xlsx(data_list, "Feb 2025 Data.xlsx")
...
no_show_rate <- access_raw_tbl %>%
  ...
  filter(appt_status %in% c("Arrived","No Show","SameDay_Canceled")) %>%
  filter(TO_DATE(date_start_formatted, "YYYY-MM-DD HH24:MI:SS") <= APPT_DTTM) %>%
  # filter(TO_DATE(date_end_formatted, "YYYY-MM-DD HH24:MI:SS") <= APPT_DTTM) %>%
  group_by(Campus, Appt.Year, Appt.MonNum, appt_status) %>%
  summarise(total = n()) %>%
  collect() %>%
  mutate(Appt.Month = format(as.Date(paste0(Appt.MonNum, "-01-", Appt.Year), "%m-%d-%Y"), "%b")) %>%
  mutate(Appt.Year = as.character(Appt.Year))
...
no_show_rate_trend_mshs <- no_show_rate %>%
  ...
  pivot_wider(names_from = appt_status,
              values_from = total,
              values_fill = 0) %>%
  mutate(total_noShow_Cancel = (`No Show` + SameDay_Canceled),
         perc_noShowPlus = round(((`No Show` + SameDay_Canceled)/(Arrived + `No Show` + SameDay_Canceled))*100,0),
         perc_noShow = round(((`No Show`)/(Arrived + `No Show` + SameDay_Canceled))*100,0),
         perc_SameDayCanceled = round(((SameDay_Canceled)/(Arrived + `No Show` + SameDay_Canceled))*100,0)) %>%
  mutate(Appt.Month = format(as.Date(paste0(Appt.MonNum, "-01-", Appt.Year), "%m-%d-%Y"), "%b")) %>%
  mutate(Appt.Year = as.character(Appt.Year)) %>%
  mutate(Appt.MonthYear = paste0(Appt.Month,"-",Appt.Year)) %>%
  arrange(Campus, Appt.Year, Appt.MonNum)
...
highchart() %>%
  hc_yAxis_multiples(
    list(title = list(text = "Total No Show/Same-Day Cancel"), lineWidth = 3),
    list(title = list(text = "% No Show"), labels = list(format = '{value}%'),
         showLastLabel = FALSE, opposite = TRUE)
  ) %>%
  hc_xAxis(title = list(text = ""),
  ...
  hc_add_series(name = "% New Visits",
                ...
                dataLabels = list(enabled = TRUE, format='{point.y}%')) %>%
  hc_legend(align = "left", verticalAlign = "top", itemStyle = list(fontFamily ='Calibri',color ='black', fontSize = "14px")) %>%
  hc_title(text = paste0(x, " - % No Show"), align = "left", style = list(fontWeight = 'bold', fontSize = "22px", fontFamily = "Calibri")) %>%
  hc_subtitle(text = paste0("Excludes Radiology and Rad Onc visits <br>", "Based on arrived visits from ", date_start, " to", date_end),
              align = "left", style = list(fontFace = "italic", fontSize = "16px", fontFamily = "Calibri")) %>%
  hc_tooltip(shared = TRUE, borderColor = "black")
}) %>%
