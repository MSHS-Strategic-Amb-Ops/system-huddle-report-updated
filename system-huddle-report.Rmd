...
#             by = c("DEPARTMENT_ID")) %>%
#   mutate(radiology_check = ifelse((DEP_CLASS == "IP_OP" & PATIENT_CLASS %LIKE% '%INPATIENT%') | (DEP_CLASS == "IP_OP" & PATIENT_CLASS %LIKE% '%EMERGENCY%'), "Exclude", "Include")) %>%
#   mutate(radiology_check = ifelse(is.na(radiology_check), "Include", radiology_check)) %>%
#   group_by(SITE, Appt.Year, Appt.MonNum, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PATIENT_CLASS, DEP_CLASS, radiology_check) %>%
#   summarise(total = n()) %>%
#   collect()
# 
# 
# scheduling_data_summary <- scheduling_data %>%
#   group_by(SITE, Appt.Year, Appt.MonNum) %>%
#   summarise(total = sum(total))
# 
# scheduling_data_filtered <- scheduling_data %>%
#   filter(radiology_check == "Include") %>%
#   group_by(SITE, Appt.Year, Appt.MonNum) %>%
#   summarise(total = sum(total))
# 
# scheduling_data_format <- scheduling_data %>%
#   pivot_wider(names_from = Appt.Year,
#               values_from = Appt.MonNum)
...
#   filter(DERIVED_STATUS_DESC == "Arrived") %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME,  DEPARTMENT_NAME, appt_year, appt_month, radiology_check, DEP_CLASS, PATIENT_CLASS) %>%
#   summarise(total = n()) %>%
#   collect()
# 
# data_list <- list("RIS Data" = radiology_data_raw,
#                   "EPIC Data" = scheduling_data)
# 
# require(openxlsx)
# write.xlsx(data_list, "Radiology Data RIS vs. EPIC Comparison.xlsx")

# check <- radiology_dep_mapping_tbl %>% collect()
...
# Format Data --------------------------
vol_trending_downtown <- access_raw_tbl %>%
  left_join(radiology_dep_mapping_tbl %>% select(DEPARTMENT_ID, DEP_CLASS),
            by = c("DEPARTMENT_ID")) %>%
  mutate(radiology_check = ifelse((DEP_CLASS == "OP") | (DEP_CLASS == "IP_OP" & PATIENT_CLASS %LIKE% '%OUTPATIENT%') | (DEP_CLASS == "IP_OP" & is.na(PATIENT_CLASS)), "Include", "Exclude")) %>%
  mutate(radiology_check = ifelse(is.na(radiology_check), "Include", radiology_check)) %>%
