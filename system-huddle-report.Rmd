...
*RIS and MOSAIC data included only in monthly volumes.*<br/>
*Includes EPIC Radiology data from May 2024~.*<br/>
...
```{r Open Encounters (5 Days) Metric Trending - Downtown, fig.width=12, fig.height = 5, fig.align = 'center', echo = FALSE, warning = FALSE, message = FALSE}
...
vollume <- access_raw_tbl %>%
  ...
  group_by(Campus, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, Appt.Year, Appt.MonNum, Visit.Method) %>%
...
```{r Volume Variance Trending by Site, echo = FALSE, warning = FALSE, message = FALSE}


# Format Data --------------------------
vol_trending <- access_raw_tbl %>%
  left_join(radiology_dep_mapping_tbl %>% select(DEPARTMENT_ID, DEP_CLASS),
            by = c("DEPARTMENT_ID")) %>%
  mutate(radiology_check = ifelse((DEP_CLASS == "OP") | (DEP_CLASS == "IP_OP" & PATIENT_CLASS %LIKE% '%OUTPATIENT%') | (DEP_CLASS == "IP_OP" & is.na(PATIENT_CLASS)), "Include", "Exclude")) %>%
  mutate(radiology_check = ifelse(is.na(radiology_check), "Include", radiology_check)) %>%
...
# vol_trending <- bind_rows(vol_trending %>% mutate(Source = "EPIC"), rad_radOnc_data %>% mutate(Source = "RIS/MOSAIC"))
...
vol_trending_downtown <- vol_trending %>%
  ...
  arrange(Campus) %>%
  mutate(perc_change = round(((`2025` - `2024`)/`2024`)*100, 0))

vol_trending_mshs <- vol_trending %>%
...
vol_trending <- bind_rows(vol_trending_mshs, vol_trending)
...
highchart() %>%
  ...
  hc_add_series(name = "2025 Total Visits",
                data   = (vol_trending %>% filter(Campus == x))$`2025`,
                marker = list(enabled= TRUE),
                color='#212070',
                lineWidth=3,
                lineColor='#212070'
...
```{r Volume Variance Trending - Downtown, echo = FALSE, warning = FALSE, message = FALSE}
...
highchart() %>%
  ...
  hc_add_series(name = "2025 Total Visits",
                data   = (vol_trending_downtown %>% filter(Campus == x))$`2025`,
                marker = list(enabled= TRUE),
                color='#212070',
                lineWidth=3,
                lineColor='#212070'
...
new_vol <- access_raw_tbl %>%
  ...
  mutate(cancel_lead_days = CONTACT_DATE - APPT_CANC_DATE) %>%
  mutate(Visit.Method  = ifelse(VISIT_METHOD == "IN PERSON", "IN PERSON", "TELEHEALTH"),
         New.PT3 = ifelse(is.na(LOS_NAME), "Established", ifelse(LOS_NAME %LIKE% "%INITIAL%" | LOS_NAME %LIKE% "%NEW%", "New", "Established")),
         enc.status = ifelse(ENC_CLOSED_CHARGE_STATUS %in% c("OPEN", "CLOSED BUT COSIGN NEEDED"), 'OPEN', 'CLOSED')) %>%
  mutate(open.days = ifelse(enc.status == "OPEN",
                            TO_DATE(lubridate::today()) - TO_DATE(CONTACT_DATE),
                            TO_DATE(Y_ENC_CLOSE_TIME) - TO_DATE(CONTACT_DATE))) %>%
  ...
  filter(DERIVED_STATUS_DESC == "Arrived") %>%
  filter(TO_DATE(date_start_formatted, "YYYY-MM-DD HH24:MI:SS") <= APPT_DTTM) %>%
  group_by(Campus, Appt.Year, Appt.MonNum, New.PT3, PROV_ID) %>%
  summarise(total = n()) %>%
  collect() %>%
...
new_vol_trending_mshs <- new_vol %>%
  ...
  mutate(Appt.Year = as.character(Appt.Year)) %>%
  mutate(Appt.MonthYear = paste0(Appt.Month,"-",Appt.Year)) %>%
  arrange(Campus, Appt.Year, Appt.MonNum)
...
new_vol_trending <- bind_rows(new_vol_trending_mshs, new_vol_trending)
...
highchart() %>%
  ...
  hc_subtitle(text = paste0("Excludes Radiology and Rad Onc visits <br>", "Based on arrived visits from ", date_start, " to", date_end),
              align = "left", style = list(fontFace = "italic", fontSize = "16px", fontFamily = "Calibri")) %>%
  hc_tooltip(shared = TRUE, borderColor = "black")
}) %>%
  hw_grid(ncol = 2, rowheight = 400) %>% htmltools::browsable()
...
--------------------------------------------------------------------------------
```{r New Patient Volume Trending - Downtown, echo = FALSE, warning = FALSE, message = FALSE}
...
new_waitTime <- access_raw_tbl %>%
  ...
  filter(TO_DATE(date_start_formatted, "YYYY-MM-DD HH24:MI:SS") <= APPT_MADE_DTTM) %>%
  filter(VISIT_GROUP_NUM == 4) %>%
  group_by(Campus, Appt.Made.Year, Appt.Made.MonNum, lead_14days) %>%
  summarise(total = n()) %>%
  collect() 


new_waitTime_trending_mshs <- new_waitTime %>%
  mutate(Campus = "MSHS: All Sites") %>%
  group_by(Campus, Appt.Made.Year, Appt.Made.MonNum, lead_14days) %>%
  summarise(total = sum(total, na.rm = T)) %>%
  pivot_wider(names_from = lead_14days,
              values_from = total,
              values_fill = 0) %>%
  rowwise() %>%
  mutate(new_scheduled = (lead_14days + lead_14days_more),
         perc_new_lead_14days = round((lead_14days/(lead_14days + lead_14days_more))*100,0)) %>%
  mutate(Appt.Made.Month = format(as.Date(paste0(Appt.Made.MonNum, "-01-", Appt.Made.Year), "%m-%d-%Y"), "%b")) %>%
  mutate(Appt.Made.Year = as.character(Appt.Made.Year)) %>%
  mutate(Appt.Made.MonthYear = paste0(Appt.Made.Month,"-",Appt.Made.Year)) %>%
  arrange(Campus, Appt.Made.Year, Appt.Made.MonNum)


new_waitTime_trending <- new_waitTime %>%
  pivot_wider(names_from = lead_14days,
              values_from = total,
              values_fill = 0) %>%
  rowwise() %>%
...
new_waitTime_trending <- bind_rows(new_waitTime_trending_mshs, new_waitTime_trending)

# Output Graphs ------------------------------
map(unique(new_waitTime_trending$Campus), function(x){
highchart() %>%
  hc_yAxis_multiples(
    list(title = list(text = "Total New Scheduled"), lineWidth = 3),
    list(title = list(text = "% of New Scheduled ≤14 Days"), labels = list(format = '{value}%'),
         showLastLabel = FALSE, opposite = TRUE)
  ) %>%
  hc_xAxis(title = list(text = ""),
           # categories = (new_waitTime_trending %>% filter(Campus == x))$Appt.MonthYear,
           categories = new_waitTime_trending$Appt.Made.MonthYear,
           type = "datetime",
           labels = list(format = '{value:%m/%d}')
           # dateTimeLabelFormats = list(month = '%Y-%m')
           ) %>%
  hc_add_series(name = "Total New Scheduled",
                data = (new_waitTime_trending %>% filter(Campus == x))$new_scheduled,
                type = 'column',
                color='#212070',
                dataLabels = list(enabled = TRUE, format='{point.y}')) %>%
  hc_add_series(name = "% of New Scheduled ≤14 Days",
                data = (new_waitTime_trending %>% filter(Campus == x))$perc_new_lead_14days,
                type = "line", yAxis = 1,
                color='#d80b8c',
                lineColor='#d80b8c',
                dataLabels = list(enabled = TRUE, format='{point.y}%')) %>%
  hc_legend(align = "left", verticalAlign = "top", itemStyle = list(fontFamily ='Calibri',color ='black', fontSize = "14px")) %>%
  hc_title(text = paste0(x, " - % New Visits Scheduled ≤14 Days"), align = "left", style = list(fontWeight = 'bold', fontSize = "22px", fontFamily = "Calibri")) %>%
  hc_subtitle(text = paste0("Excludes Radiology and Rad Onc visits <br>", "Based on visits scheduled from ", date_start, " to", date_end),
              align = "left", style = list(fontFace = "italic", fontSize = "16px", fontFamily = "Calibri")) %>%
  hc_tooltip(shared = TRUE, borderColor = "black")
}) %>%
  hw_grid(ncol = 2, rowheight = 450) %>% htmltools::browsable()
...
```{r Time to Appointment Metrics - Downtown, echo = FALSE, warning = FALSE, message = FALSE}

new_waitTime_trending_downtown <- new_waitTime %>%
  filter(Campus %in% c("MSUS","MSDMG","MSBI")) %>%
  mutate(Campus = "MS Downtown") %>%
  group_by(Campus, Appt.Made.Year, Appt.Made.MonNum, lead_14days) %>%
  summarise(total = sum(total, na.rm = T)) %>%
...
# Output Graphs ------------------------------
map(unique(new_waitTime_trending_downtown$Campus), function(x){
highchart() %>%
  hc_yAxis_multiples(
    list(title = list(text = "Total New Scheduled"), lineWidth = 3),
    list(title = list(text = "% of New Scheduled ≤14 Days"), labels = list(format = '{value}%'),
  ...
  hc_xAxis(title = list(text = ""),
           # categories = (new_waitTime_trending_downtown %>% filter(Campus == x))$Appt.MonthYear,
           categories = new_waitTime_trending_downtown$Appt.Made.MonthYear,
           type = "datetime",
           labels = list(format = '{value:%m/%d}')
           # dateTimeLabelFormats = list(month = '%Y-%m')
           ) %>%
  hc_add_series(name = "Total New Scheduled",
                data = (new_waitTime_trending_downtown %>% filter(Campus == x))$new_scheduled,
                type = 'column',
                color='#212070',
                dataLabels = list(enabled = TRUE, format='{point.y}')) %>%
  hc_add_series(name = "% of New Scheduled ≤14 Days",
                data = (new_waitTime_trending_downtown %>% filter(Campus == x))$perc_new_lead_14days,
                type = "line", yAxis = 1,
                color='#d80b8c',
                lineColor='#d80b8c',
                dataLabels = list(enabled = TRUE, format='{point.y}%')) %>%
...
no_show_rate <- access_raw_tbl %>%
  ...
  filter(TO_DATE(date_start_formatted, "YYYY-MM-DD HH24:MI:SS") <= APPT_DTTM) %>%
  filter(TO_DATE(date_end_formatted, "YYYY-MM-DD HH24:MI:SS") > APPT_DTTM) %>%
  group_by(Campus, Appt.Year, Appt.MonNum, DERIVED_STATUS_DESC) %>%
  summarise(total = n()) %>%
  collect() %>%
  mutate(Appt.Month = format(as.Date(paste0(Appt.MonNum, "-01-", Appt.Year), "%m-%d-%Y"), "%b")) %>%
  mutate(Appt.Year = as.character(Appt.Year))
...
no_show_rate_trend_mshs <- no_show_rate %>%
  mutate(Campus = "MSHS: All Sites") %>%
  group_by(Campus, Appt.Year, Appt.MonNum, DERIVED_STATUS_DESC) %>%
  summarise(total = sum(total, na.rm = T)) %>%
  pivot_wider(names_from = DERIVED_STATUS_DESC,
              values_from = total,
              values_fill = 0) %>%
  mutate(total_noShow_Cancel = Arrived + `No Show` + Canceled,
         perc_noShowPlusPlus = round(((`No Show` + Canceled + Rescheduled)/(Arrived + `No Show` + Canceled + Rescheduled))*100,0),
         perc_noShowPlus =  round(((`No Show` + Canceled)/(Arrived + `No Show` + Canceled))*100,0),
         perc_noShow =  round(((`No Show`)/(Arrived + `No Show`))*100,0)) %>%
  mutate(Appt.Month = format(as.Date(paste0(Appt.MonNum, "-01-", Appt.Year), "%m-%d-%Y"), "%b")) %>%
  mutate(Appt.Year = as.character(Appt.Year)) %>%
  mutate(Appt.MonthYear = paste0(Appt.Month,"-",Appt.Year)) %>%
  arrange(Campus, Appt.Year, Appt.MonNum)
...
no_show_rate_trend <- no_show_rate %>%
  group_by(Campus, Appt.Year, Appt.MonNum, DERIVED_STATUS_DESC) %>%
  summarise(total = sum(total, na.rm = T)) %>%
  pivot_wider(names_from = DERIVED_STATUS_DESC,
              values_from = total,
...
no_show_rate_trend <- bind_rows(no_show_rate_trend_mshs, no_show_rate_trend)

# Output Graphs ------------------------------
map(unique(no_show_rate_trend$Campus), function(x){
highchart() %>%
  hc_yAxis_multiples(
    list(title = list(text = "No Show and Late-Canceled"), lineWidth = 3),
    list(title = list(text = "No Show Plus Rate"), labels = list(format = '{value}%'),
         showLastLabel = FALSE, opposite = TRUE)
  ) %>%
  hc_xAxis(title = list(text = ""),
           # categories = (no_show_rate_trend %>% filter(Campus == x))$Appt.MonthYear,
           categories = no_show_rate_trend$Appt.MonthYear,
           type = "datetime",
           labels = list(format = '{value:%m/%d}')
           # dateTimeLabelFormats = list(month = '%Y-%m')
           ) %>%
  hc_add_series(name = "No Show and Late-Canceled",
                data = (no_show_rate_trend %>% filter(Campus == x))$total_noShow_Cancel,
                type = 'column',
                color='#212070',
                dataLabels = list(enabled = TRUE, format='{point.y}')) %>%
  hc_add_series(name = "No Show Plus Rate",
                data = (no_show_rate_trend %>% filter(Campus == x))$perc_noShowPlus,
                type = "line", yAxis = 1,
                color='#d80b8c',
                lineColor='#d80b8c',
                dataLabels = list(enabled = TRUE, format='{point.y}%')) %>%
  hc_legend(align = "left", verticalAlign = "top", itemStyle = list(fontFamily ='Calibri',color ='black', fontSize = "14px")) %>%
  hc_title(text = paste0(x, " - No Show Plus Rate"), align = "left", style = list(fontWeight = 'bold', fontSize = "22px", fontFamily = "Calibri")) %>%
  hc_subtitle(text = paste0("Excludes Radiology and Rad Onc visits <br>", "Based on arrived visits from ", date_start, " to", date_end),
              align = "left", style = list(fontFace = "italic", fontSize = "16px", fontFamily = "Calibri")) %>%
  hc_tooltip(shared = TRUE, borderColor = "black")
}) %>%
...
```{r No Show Metric Trending - Downtown, fig.width=12, fig.height = 5, fig.align = 'center', echo = FALSE, warning = FALSE, message = FALSE}

no_show_rate_trend_downtown <- no_show_rate %>%
  filter(Campus %in% c("MSUS","MSDMG","MSBI")) %>%
  mutate(Campus = "MS Downtown") %>%
  group_by(Campus, Appt.Year, Appt.MonNum, DERIVED_STATUS_DESC) %>%
  summarise(total = sum(total, na.rm = T)) %>%
...
# Output Graphs ------------------------------
map(unique(no_show_rate_trend_downtown$Campus), function(x){
highchart() %>%
  hc_yAxis_multiples(
    list(title = list(text = "No Show and Late-Canceled"), lineWidth = 3),
    list(title = list(text = "No Show Plus Rate"), labels = list(format = '{value}%'),
  ...
  hc_xAxis(title = list(text = ""),
           # categories = (no_show_rate_trend_downtown %>% filter(Campus == x))$Appt.MonthYear,
           categories = no_show_rate_trend_downtown$Appt.MonthYear,
           type = "datetime",
           labels = list(format = '{value:%m/%d}')
           # dateTimeLabelFormats = list(month = '%Y-%m')
           ) %>%
  hc_add_series(name = "No Show and Late-Canceled",
                data = (no_show_rate_trend_downtown %>% filter(Campus == x))$total_noShow_Cancel,
                type = 'column',
                color='#212070',
                dataLabels = list(enabled = TRUE, format='{point.y}')) %>%
  hc_add_series(name = "No Show Plus Rate",
                data = (no_show_rate_trend_downtown %>% filter(Campus == x))$perc_noShowPlus,
                type = "line", yAxis = 1,
                color='#d80b8c',
                lineColor='#d80b8c',
                dataLabels = list(enabled = TRUE, format='{point.y}%')) %>%
...
hh_site_mshs <- hh_site %>%
  filter(!is.na(unit)) %>%
  mutate(site = "MSHS: All Sites") %>%
  group_by(site, Month) %>%
  summarise(worker___1 = sum(worker___1),
            worker___2 = sum(worker___2),
            worker___3 = sum(worker___3),
...
hh_site <- bind_rows(hh_site_mshs, hh_site)
...
--------------------------------------------------------------------------------
```{r Hand Hygiene Trending - Downtown, echo = FALSE, warning = FALSE, message = FALSE}
